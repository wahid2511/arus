...
void main() {
  test(
    'downloads a byte-identical segmented file into a missing directory',
    () async {
      ...
      final target = joinPath(
        joinPath(root.path, 'nested', 'folder'),
        'sample.bin',
      );
  ...
  test(
    'resumes a previous segmented checkpoint and removes it after assembly',
    () async {
      final bytes = List<int>.generate(
        96 * 1024,
        (index) => (index * 13) % 239,
  ...
  test(
    'falls back to a single stream when the server ignores ranges',
    () async {
  ...
  test(
    'rebuilds successfully when Range support disappears after the probe',
    () async {
      final bytes = List<int>.generate(
        128 * 1024,
        (index) => (index * 23) % 241,
  ...
  test(
    'retries a dropped single stream and resumes the partial file',
    () async {
      final bytes = List<int>.generate(
        48 * 1024,
        (index) => (index * 17) % 233,
  ...
  server.listen((request) async {
    ...
    if (supportsRanges && range != null) {
      ...
      if (match != null) {
        ...
        final requestedEnd = match.group(2) == null
            ? bytes.length - 1
